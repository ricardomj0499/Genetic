...
public class Metadata{
    ...
    public static BufferedImage getting_metadata(BufferedImage imagen, int[][] prob_x_cuadrante){
        ...
        for(int x=0; x<=896; x+=128){//x se refiere a la coord x de la imagen, se llega hsta 896 para no salirse de los lim
            ...
            for(int y = 0; y<=896; y+=128){//a la hora de producir random. Es lo mismo con la y
                ...
                for(int cont_random=0; cont_random<50*prob_x_cuadrante[num_cuadranteX][num_cuadranteY]; cont_random++){//32768//mitad de cada subcuadro
                    ...
                    //double rrr = Math.floor(Math.random()*16500000);  // Valor entre M y N, ambos incluidos
                    // int eee= (int)Math.round(rrr);
                    imagen.setRGB(x1, y1, 0);
                    //}else{
                      //  System.out.println("blanco");
                    //}
                }
                cuadrante++;
                System.out.println(media_RGB[0]+","+media_RGB[1]+","+media_RGB[1]);
                media_RGB[0] = (int)(media_RGB[0]/50*prob_x_cuadrante[num_cuadranteX][num_cuadranteY]);
                media_RGB[1] =(int)(media_RGB[1]/50*prob_x_cuadrante[num_cuadranteX][num_cuadranteY]);
                media_RGB[2] = (int)(media_RGB[2]/50*prob_x_cuadrante[num_cuadranteX][num_cuadranteY]);
                if(media_RGB[0]>=250 && media_RGB[1]>=250 && media_RGB[2]>=250){
                    prob_x_cuadrante[num_cuadranteX][num_cuadranteY] = (int) (prob_x_cuadrante[num_cuadranteX][num_cuadranteY]-0.10);
                }/*else if((media_RGB[0]<253 && media_RGB[0]>=200) && (media_RGB[1]<253 && media_RGB[1]>=200) && (media_RGB[2]<253 && media_RGB[2]>=200)){
                    prob_x_cuadrante[num_cuadranteX][num_cuadranteY] = (int) (prob_x_cuadrante[num_cuadranteX][num_cuadranteY]*1.25);
                }*/else{
